...
/Temp/NetCodeGenerated/Assembly-CSharp/2062241058503599662_ChunkTilesSerializer.cs"
...
namespace Assembly_CSharp_Generated
{
    ...
    /// </summary>
    public partial struct Assembly_CSharp_Generated_ChunkTilesGhostComponentSerializer : IGhostSerializer
    {
        /// <summary>
        ...
        static void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, ref ChunkTiles component)
        {
                snapshot.tileID = (int) component.tileID;
                snapshot.variant = (uint)component.variant;
        }

        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.CopyFromSnapshotGenerated"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref ChunkTiles component,
        ...
        {
                component.tileID = (int) snapshotBefore.tileID;
                component.variant = (byte) snapshotBefore.variant;
        }

        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.RestoreFromBackupGenerated"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void RestoreFromBackupGenerated(ref ChunkTiles component, ref ChunkTiles backup)
        {
            component.tileID = backup.tileID;
            component.variant = backup.variant;
        }
        ...
        {
            snapshot.tileID = predictor.PredictInt(snapshot.tileID, baseline1.tileID, baseline2.tileID);
            snapshot.variant = (uint)predictor.PredictInt((int)snapshot.variant, (int)baseline1.variant, (int)baseline2.variant);
        }
        ...
        {
            uint changeMask = 0;
            changeMask = (snapshot.tileID != baseline.tileID) ? 1u : 0;
            changeMask |= (snapshot.variant != baseline.variant) ? (1u<<1) : 0;
            ...
            ref DataStreamWriter writer, in StreamCompressionModel compressionModel)
        {
            ...
            if ((changeMask & (1 << 0)) != 0)
                writer.WritePackedIntDelta(snapshot.tileID, baseline.tileID, compressionModel);
            if ((changeMask & (1 << 1)) != 0)
                writer.WritePackedUIntDelta(snapshot.variant, baseline.variant, compressionModel);
        }
            ...
            IntPtr changeMaskData, int startOffset, ref Snapshot snapshot, ref Snapshot baseline)
        {
            uint changeMask = GhostComponentSerializer.CopyFromChangeMask(changeMaskData, startOffset, ChangeMaskBits);
            if ((changeMask & (1 << 0)) != 0)
                snapshot.tileID = reader.ReadPackedIntDelta(baseline.tileID, compressionModel);
            else
                snapshot.tileID = baseline.tileID;
            if ((changeMask & (1 << 1)) != 0)
                snapshot.variant = reader.ReadPackedUIntDelta(baseline.variant, compressionModel);
            else
                snapshot.variant = baseline.variant;
        }

#if UNITY_EDITOR || NETCODE_DEBUG
        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.ReportPredictionErrorsGenerated"/>
        static void ReportPredictionErrorsGenerated(in ChunkTiles component, in ChunkTiles backup, IntPtr errorsList,
            int errorsCount)
        {
            var errors = GhostComponentSerializer.ConvertToUnsafeList(errorsList, errorsCount);
            int errorIndex = 0;
            errors[errorIndex] = math.max(errors[errorIndex], math.abs(component.tileID - backup.tileID));
            ++errorIndex;
            errors[errorIndex] = math.max(errors[errorIndex],
                (component.variant > backup.variant) ?
                (component.variant - backup.variant) :
                (backup.variant - component.variant));
            ++errorIndex;
        }
        ...
        static internal int GetPredictionErrorNamesGenerated(ref FixedString512Bytes names)
        {
            ...
            names.Append((FixedString512Bytes)".tileID");
            ++nameCount;
            if (nameCount != 0)
                names.Append(new FixedString32Bytes(","));
            names.Append((FixedString512Bytes)".variant");
            ++nameCount;
            return nameCount;
        }
#endif
        ...
        public void CopyToSnapshot(in GhostSerializerState serializerState, [NoAlias]IntPtr snapshot, [NoAlias][ReadOnly]IntPtr component)
        {
            CopyToSnapshotGenerated(serializerState, ref GhostComponentSerializer.TypeCast<Snapshot>(snapshot),
                ref GhostComponentSerializer.TypeCast<ChunkTiles>(component));
        }
        ...
        public void RestoreFromBackup([NoAlias] IntPtr component, [NoAlias] [ReadOnly] IntPtr backup)
        {
            RestoreFromBackupGenerated(
                ref GhostComponentSerializer.TypeCast<ChunkTiles>(component),
                ref GhostComponentSerializer.TypeCast<ChunkTiles>(backup));
        }
        ...
        public void ReportPredictionErrors([NoAlias][ReadOnly]IntPtr component, [NoAlias][ReadOnly]IntPtr backup, IntPtr errorsList,
            int errorsCount)
        {
            ReportPredictionErrorsGenerated(GhostComponentSerializer.TypeCast<ChunkTiles>(component),
                GhostComponentSerializer.TypeCast<ChunkTiles>(backup), errorsList, errorsCount);
        }
#endif
    }
    ...
    public partial struct Assembly_CSharp_Generated_ChunkTilesGhostComponentSerializer : IGhostSerializer
    {
        static internal GhostComponentSerializer.State GetState(ref SystemState state)
        {
            ...
            if (!s_StateInitialized)
            {
                s_State = new GhostComponentSerializer.State
                {
                    ...
                    ComponentType = ComponentType.ReadWrite<ChunkTiles>(),
                    ComponentSize = UnsafeUtility.SizeOf<ChunkTiles>(),
#if COMPONENT_HAS_GHOST_FIELDS
                    ...
                    VariantHash = 13590023896616619412,
                    SerializationStrategyIndex = -1,
                    SerializesEnabledBit = 0,
#if UNITY_EDITOR || NETCODE_DEBUG
                    ProfilerMarker = new Unity.Profiling.ProfilerMarker("ChunkTiles"),
                    VariantTypeFullNameHash = TypeHash.FNV1A64("ChunkTiles")
#endif
                };
                ...
                var innerMarker = new Unity.Profiling.ProfilerMarker("Assembly_CSharp_Generated_ChunkTiles");
                innerMarker.Begin();
#endif
        ...
        {
#if COMPONENT_HAS_GHOST_FIELDS
            var serializer = default(Assembly_CSharp_Generated_ChunkTilesGhostComponentSerializer);
            Unity.NetCode.ComponentSerializationHelper<
                    ChunkTiles,
                    Snapshot,
                    Assembly_CSharp_Generated_ChunkTilesGhostComponentSerializer
