...
/Temp/NetCodeGenerated/Assembly-CSharp/2062241058503599662_EquipmentEventBufferSerializer.cs"
...
namespace Assembly_CSharp_Generated
{
    ...
    /// </summary>
    public partial struct Assembly_CSharp_Generated_EquipmentEventBufferGhostComponentSerializer : IGhostSerializer
    {
        /// <summary>
        ...
        static void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, ref EquipmentEventBuffer component)
        {
                snapshot.data_slot = (int) component.data.slot;
                snapshot.data_flags = (uint)component.data.flags;
                snapshot.data_value = (int) component.data.value;
                snapshot.index = (uint)component.index;
        }

        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.CopyFromSnapshotGenerated"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref EquipmentEventBuffer component,
        ...
        {
                component.data.slot = (int) snapshotBefore.data_slot;
                component.data.flags = (byte) snapshotBefore.data_flags;
                component.data.value = (int) snapshotBefore.data_value;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void RestoreFromBackupGenerated(ref EquipmentEventBuffer component, ref EquipmentEventBuffer backup)
        {
            component.data.slot = backup.data.slot;
            component.data.flags = backup.data.flags;
            component.data.value = backup.data.value;
        ...
        {
            snapshot.data_slot = predictor.PredictInt(snapshot.data_slot, baseline1.data_slot, baseline2.data_slot);
            snapshot.data_flags = (uint)predictor.PredictInt((int)snapshot.data_flags, (int)baseline1.data_flags, (int)baseline2.data_flags);
            snapshot.data_value = predictor.PredictInt(snapshot.data_value, baseline1.data_value, baseline2.data_value);
        ...
        {
            uint changeMask = 0;
            changeMask = (snapshot.data_slot != baseline.data_slot) ? 1u : 0;
            changeMask |= (snapshot.data_flags != baseline.data_flags) ? (1u<<1) : 0;
            changeMask |= (snapshot.data_value != baseline.data_value) ? (1u<<2) : 0;
            ...
            ref DataStreamWriter writer, in StreamCompressionModel compressionModel)
        {
            ...
            if ((changeMask & (1 << 0)) != 0)
                writer.WritePackedIntDelta(snapshot.data_slot, baseline.data_slot, compressionModel);
            if ((changeMask & (1 << 1)) != 0)
                writer.WritePackedUIntDelta(snapshot.data_flags, baseline.data_flags, compressionModel);
            if ((changeMask & (1 << 2)) != 0)
                writer.WritePackedIntDelta(snapshot.data_value, baseline.data_value, compressionModel);
            if ((changeMask & (1 << 3)) != 0)
            ...
            IntPtr changeMaskData, int startOffset, ref Snapshot snapshot, ref Snapshot baseline)
        {
            uint changeMask = GhostComponentSerializer.CopyFromChangeMask(changeMaskData, startOffset, ChangeMaskBits);
            if ((changeMask & (1 << 0)) != 0)
                snapshot.data_slot = reader.ReadPackedIntDelta(baseline.data_slot, compressionModel);
            else
                snapshot.data_slot = baseline.data_slot;
            if ((changeMask & (1 << 1)) != 0)
                snapshot.data_flags = reader.ReadPackedUIntDelta(baseline.data_flags, compressionModel);
            else
                snapshot.data_flags = baseline.data_flags;
            if ((changeMask & (1 << 2)) != 0)
                snapshot.data_value = reader.ReadPackedIntDelta(baseline.data_value, compressionModel);
            else
                snapshot.data_value = baseline.data_value;
            if ((changeMask & (1 << 3)) != 0)
        ...
        static void ReportPredictionErrorsGenerated(in EquipmentEventBuffer component, in EquipmentEventBuffer backup, IntPtr errorsList,
            int errorsCount)
        {
            var errors = GhostComponentSerializer.ConvertToUnsafeList(errorsList, errorsCount);
            int errorIndex = 0;
            errors[errorIndex] = math.max(errors[errorIndex], math.abs(component.data.slot - backup.data.slot));
            ++errorIndex;
            errors[errorIndex] = math.max(errors[errorIndex],
                (component.data.flags > backup.data.flags) ?
                (component.data.flags - backup.data.flags) :
                (backup.data.flags - component.data.flags));
            ++errorIndex;
            errors[errorIndex] = math.max(errors[errorIndex], math.abs(component.data.value - backup.data.value));
            ++errorIndex;
            errors[errorIndex] = math.max(errors[errorIndex],
                (component.index > backup.index) ?
                (component.index - backup.index) :
                (backup.index - component.index));
            ++errorIndex;
        }
        ...
        static internal int GetPredictionErrorNamesGenerated(ref FixedString512Bytes names)
        {
            ...
            names.Append((FixedString512Bytes)".data.slot");
            ++nameCount;
            if (nameCount != 0)
                names.Append(new FixedString32Bytes(","));
            names.Append((FixedString512Bytes)".data.flags");
            ++nameCount;
            if (nameCount != 0)
                names.Append(new FixedString32Bytes(","));
            names.Append((FixedString512Bytes)".data.value");
            ++nameCount;
            if (nameCount != 0)
                names.Append(new FixedString32Bytes(","));
        ...
        public void CopyToSnapshot(in GhostSerializerState serializerState, [NoAlias]IntPtr snapshot, [NoAlias][ReadOnly]IntPtr component)
        {
            CopyToSnapshotGenerated(serializerState, ref GhostComponentSerializer.TypeCast<Snapshot>(snapshot),
                ref GhostComponentSerializer.TypeCast<EquipmentEventBuffer>(component));
        }
        ...
        public void RestoreFromBackup([NoAlias] IntPtr component, [NoAlias] [ReadOnly] IntPtr backup)
        {
            RestoreFromBackupGenerated(
                ref GhostComponentSerializer.TypeCast<EquipmentEventBuffer>(component),
                ref GhostComponentSerializer.TypeCast<EquipmentEventBuffer>(backup));
        }
        ...
        public void ReportPredictionErrors([NoAlias][ReadOnly]IntPtr component, [NoAlias][ReadOnly]IntPtr backup, IntPtr errorsList,
            int errorsCount)
        {
            ReportPredictionErrorsGenerated(GhostComponentSerializer.TypeCast<EquipmentEventBuffer>(component),
                GhostComponentSerializer.TypeCast<EquipmentEventBuffer>(backup), errorsList, errorsCount);
        }
#endif
    }
    ...
    public partial struct Assembly_CSharp_Generated_EquipmentEventBufferGhostComponentSerializer : IGhostSerializer
    {
        static internal GhostComponentSerializer.State GetState(ref SystemState state)
        {
            ...
            if (!s_StateInitialized)
            {
                s_State = new GhostComponentSerializer.State
                {
                    ...
                    ComponentType = ComponentType.ReadWrite<EquipmentEventBuffer>(),
                    ComponentSize = UnsafeUtility.SizeOf<EquipmentEventBuffer>(),
#if COMPONENT_HAS_GHOST_FIELDS
                    ...
                    VariantHash = 8390267636598729308,
                    SerializationStrategyIndex = -1,
                    SerializesEnabledBit = 0,
#if UNITY_EDITOR || NETCODE_DEBUG
                    ProfilerMarker = new Unity.Profiling.ProfilerMarker("EquipmentEventBuffer"),
                    VariantTypeFullNameHash = TypeHash.FNV1A64("EquipmentEventBuffer")
#endif
                };
                ...
                var innerMarker = new Unity.Profiling.ProfilerMarker("Assembly_CSharp_Generated_EquipmentEventBuffer");
                innerMarker.Begin();
#endif
        ...
        {
#if COMPONENT_HAS_GHOST_FIELDS
            var serializer = default(Assembly_CSharp_Generated_EquipmentEventBufferGhostComponentSerializer);
            Unity.NetCode.ComponentSerializationHelper<
                    EquipmentEventBuffer,
                    Snapshot,
                    Assembly_CSharp_Generated_EquipmentEventBufferGhostComponentSerializer
