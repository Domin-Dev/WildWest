...
/Temp/NetCodeGenerated/Assembly-CSharp/2062241058503599662_ItemBarDataSerializer.cs"
...
namespace Assembly_CSharp_Generated
{
    ...
    /// </summary>
    public partial struct Assembly_CSharp_Generated_ItemBarDataGhostComponentSerializer : IGhostSerializer
    {
        /// <summary>
        ...
        static void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, ref ItemBarData component)
        {
                snapshot.slot = (int) component.slot;
                snapshot.value = component.value;
                snapshot.maxValue = component.maxValue;
        }

        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.CopyFromSnapshotGenerated"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref ItemBarData component,
            float snapshotInterpolationFactor, float snapshotInterpolationFactorRaw, ref Snapshot snapshotBefore, ref Snapshot snapshotAfter)
        {
                ...
                component.value = snapshotBefore.value;
                component.maxValue = snapshotBefore.maxValue;
        }

        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.RestoreFromBackupGenerated"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void RestoreFromBackupGenerated(ref ItemBarData component, ref ItemBarData backup)
        {
            component.slot = backup.slot;
            component.value = backup.value;
            component.maxValue = backup.maxValue;
        }
            ...
            [NoAlias]IntPtr changeMaskData, int startOffset)
        {
            ...
            changeMask |= (snapshot.value != baseline.value) ? (1u<<1) : 0;
            changeMask |= (snapshot.maxValue != baseline.maxValue) ? (1u<<2) : 0;
            ...
            ref DataStreamWriter writer, in StreamCompressionModel compressionModel)
        {
            ...
            if ((changeMask & (1 << 1)) != 0)
                writer.WritePackedFloatDelta(snapshot.value, baseline.value, compressionModel);
            if ((changeMask & (1 << 2)) != 0)
                writer.WritePackedFloatDelta(snapshot.maxValue, baseline.maxValue, compressionModel);
        }
            ...
            IntPtr changeMaskData, int startOffset, ref Snapshot snapshot, ref Snapshot baseline)
        {
            ...
            if ((changeMask & (1 << 1)) != 0)
                snapshot.value = reader.ReadPackedFloatDelta(baseline.value, compressionModel);
            else
                snapshot.value = baseline.value;
            if ((changeMask & (1 << 2)) != 0)
                snapshot.maxValue = reader.ReadPackedFloatDelta(baseline.maxValue, compressionModel);
            else
                snapshot.maxValue = baseline.maxValue;
        }

#if UNITY_EDITOR || NETCODE_DEBUG
        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.ReportPredictionErrorsGenerated"/>
        static void ReportPredictionErrorsGenerated(in ItemBarData component, in ItemBarData backup, IntPtr errorsList,
            int errorsCount)
        {
            ...
            ++errorIndex;
            errors[errorIndex] = math.max(errors[errorIndex], math.abs(component.maxValue - backup.maxValue));
            ++errorIndex;
        }
        ...
        static internal int GetPredictionErrorNamesGenerated(ref FixedString512Bytes names)
        {
            ...
            if (nameCount != 0)
                names.Append(new FixedString32Bytes(","));
            names.Append((FixedString512Bytes)".value");
            ++nameCount;
            if (nameCount != 0)
                names.Append(new FixedString32Bytes(","));
            names.Append((FixedString512Bytes)".maxValue");
            ++nameCount;
            return nameCount;
        }
#endif
        ...
        public void CopyToSnapshot(in GhostSerializerState serializerState, [NoAlias]IntPtr snapshot, [NoAlias][ReadOnly]IntPtr component)
        {
            CopyToSnapshotGenerated(serializerState, ref GhostComponentSerializer.TypeCast<Snapshot>(snapshot),
                ref GhostComponentSerializer.TypeCast<ItemBarData>(component));
        }
        ...
        public void RestoreFromBackup([NoAlias] IntPtr component, [NoAlias] [ReadOnly] IntPtr backup)
        {
            RestoreFromBackupGenerated(
                ref GhostComponentSerializer.TypeCast<ItemBarData>(component),
                ref GhostComponentSerializer.TypeCast<ItemBarData>(backup));
        }
        ...
        public void ReportPredictionErrors([NoAlias][ReadOnly]IntPtr component, [NoAlias][ReadOnly]IntPtr backup, IntPtr errorsList,
            int errorsCount)
        {
            ReportPredictionErrorsGenerated(GhostComponentSerializer.TypeCast<ItemBarData>(component),
                GhostComponentSerializer.TypeCast<ItemBarData>(backup), errorsList, errorsCount);
        }
#endif
    }
    ...
    public partial struct Assembly_CSharp_Generated_ItemBarDataGhostComponentSerializer : IGhostSerializer
    {
        static internal GhostComponentSerializer.State GetState(ref SystemState state)
        {
            ...
            if (!s_StateInitialized)
            {
                s_State = new GhostComponentSerializer.State
                {
                    ...
                    ComponentType = ComponentType.ReadWrite<ItemBarData>(),
                    ComponentSize = UnsafeUtility.SizeOf<ItemBarData>(),
#if COMPONENT_HAS_GHOST_FIELDS
                    ...
                    VariantHash = 16658401302825848676,
                    SerializationStrategyIndex = -1,
                    SerializesEnabledBit = 0,
#if UNITY_EDITOR || NETCODE_DEBUG
                    ProfilerMarker = new Unity.Profiling.ProfilerMarker("ItemBarData"),
                    VariantTypeFullNameHash = TypeHash.FNV1A64("ItemBarData")
#endif
                };
                ...
                var innerMarker = new Unity.Profiling.ProfilerMarker("Assembly_CSharp_Generated_ItemBarData");
                innerMarker.Begin();
#endif
        ...
        {
#if COMPONENT_HAS_GHOST_FIELDS
            var serializer = default(Assembly_CSharp_Generated_ItemBarDataGhostComponentSerializer);
            Unity.NetCode.ComponentSerializationHelper<
                    ItemBarData,
                    Snapshot,
                    Assembly_CSharp_Generated_ItemBarDataGhostComponentSerializer
