...
/Temp/NetCodeGenerated/Assembly-CSharp/2062241058503599662_BulletSerializer.cs"
...
namespace Assembly_CSharp_Generated
{
    ...
    /// </summary>
    public partial struct Assembly_CSharp_Generated_BulletGhostComponentSerializer : IGhostSerializer
    {
        /// <summary>
        ...
        static void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, ref Bullet component)
        {
                snapshot.bulletID = (uint)component.bulletID;
        }

        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.CopyFromSnapshotGenerated"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref Bullet component,
        ...
        {
                component.bulletID = (uint) snapshotBefore.bulletID;
        }

        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.RestoreFromBackupGenerated"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void RestoreFromBackupGenerated(ref Bullet component, ref Bullet backup)
        {
            component.bulletID = backup.bulletID;
        }
        ...
        {
            snapshot.bulletID = (uint)predictor.PredictInt((int)snapshot.bulletID, (int)baseline1.bulletID, (int)baseline2.bulletID);
        }
        ...
        {
            uint changeMask = 0;
            changeMask = (snapshot.bulletID != baseline.bulletID) ? 1u : 0;
            ...
            ref DataStreamWriter writer, in StreamCompressionModel compressionModel)
        {
            ...
            if ((changeMask & (1 << 0)) != 0)
                writer.WritePackedUIntDelta(snapshot.bulletID, baseline.bulletID, compressionModel);
        }
            ...
            IntPtr changeMaskData, int startOffset, ref Snapshot snapshot, ref Snapshot baseline)
        {
            uint changeMask = GhostComponentSerializer.CopyFromChangeMask(changeMaskData, startOffset, ChangeMaskBits);
            if ((changeMask & (1 << 0)) != 0)
                snapshot.bulletID = reader.ReadPackedUIntDelta(baseline.bulletID, compressionModel);
            else
                snapshot.bulletID = baseline.bulletID;
        }

#if UNITY_EDITOR || NETCODE_DEBUG
        /// <inheritdoc cref="IGhostSerializer{TComponent,TSnapshot}.ReportPredictionErrorsGenerated"/>
        static void ReportPredictionErrorsGenerated(in Bullet component, in Bullet backup, IntPtr errorsList,
            int errorsCount)
        {
            ...
            errors[errorIndex] = math.max(errors[errorIndex],
                (component.bulletID > backup.bulletID) ?
                (component.bulletID - backup.bulletID) :
                (backup.bulletID - component.bulletID));
            ++errorIndex;
        }
        ...
        static internal int GetPredictionErrorNamesGenerated(ref FixedString512Bytes names)
        {
            ...
            names.Append((FixedString512Bytes)".bulletID");
            ++nameCount;
            return nameCount;
        }
#endif
        ...
        public void CopyToSnapshot(in GhostSerializerState serializerState, [NoAlias]IntPtr snapshot, [NoAlias][ReadOnly]IntPtr component)
        {
            CopyToSnapshotGenerated(serializerState, ref GhostComponentSerializer.TypeCast<Snapshot>(snapshot),
                ref GhostComponentSerializer.TypeCast<Bullet>(component));
        }
        ...
        public void RestoreFromBackup([NoAlias] IntPtr component, [NoAlias] [ReadOnly] IntPtr backup)
        {
            RestoreFromBackupGenerated(
                ref GhostComponentSerializer.TypeCast<Bullet>(component),
                ref GhostComponentSerializer.TypeCast<Bullet>(backup));
        }
        ...
        public void ReportPredictionErrors([NoAlias][ReadOnly]IntPtr component, [NoAlias][ReadOnly]IntPtr backup, IntPtr errorsList,
            int errorsCount)
        {
            ReportPredictionErrorsGenerated(GhostComponentSerializer.TypeCast<Bullet>(component),
                GhostComponentSerializer.TypeCast<Bullet>(backup), errorsList, errorsCount);
        }
#endif
    }
    ...
    public partial struct Assembly_CSharp_Generated_BulletGhostComponentSerializer : IGhostSerializer
    {
        static internal GhostComponentSerializer.State GetState(ref SystemState state)
        {
            ...
            if (!s_StateInitialized)
            {
                s_State = new GhostComponentSerializer.State
                {
                    ...
                    ComponentType = ComponentType.ReadWrite<Bullet>(),
                    ComponentSize = UnsafeUtility.SizeOf<Bullet>(),
#if COMPONENT_HAS_GHOST_FIELDS
                    ...
                    VariantHash = 10237261704927524668,
                    SerializationStrategyIndex = -1,
                    SerializesEnabledBit = 0,
#if UNITY_EDITOR || NETCODE_DEBUG
                    ProfilerMarker = new Unity.Profiling.ProfilerMarker("Bullet"),
                    VariantTypeFullNameHash = TypeHash.FNV1A64("Bullet")
#endif
                };
                ...
                var innerMarker = new Unity.Profiling.ProfilerMarker("Assembly_CSharp_Generated_Bullet");
                innerMarker.Begin();
#endif
        ...
        {
#if COMPONENT_HAS_GHOST_FIELDS
            var serializer = default(Assembly_CSharp_Generated_BulletGhostComponentSerializer);
            Unity.NetCode.ComponentSerializationHelper<
                    Bullet,
                    Snapshot,
                    Assembly_CSharp_Generated_BulletGhostComponentSerializer
