...
partial struct CalculateChunksForPlayersServerSystem : ISystem
{
    ...
    {
        playersQuery = SystemAPI.QueryBuilder().WithAll<GhostChunk, GhostOwner, Player, PlayerChunks>()
        .WithPresent<NewChunk>() // <-- tylko encje z włączonym NewChunk
        .Build();
        //playersQuery = SystemAPI.QueryBuilder().WithAll<GhostChunk, NewChunk, GhostOwner, Player, PlayerChunks>().wit.Build();
